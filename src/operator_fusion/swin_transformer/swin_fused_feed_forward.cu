...
#include "kernels/fused_feed_forward_fc1_fc2_m4096_n512_k128.cu"
#include "kernels/fused_feed_forward_fc1_fc2_n128_k128_n512_k128.cu"

template <int64_t B, int64_t M, int64_t N, int64_t K>
float test_fused_feed_forward(int round_cout = 1, int loop = 1, int func_id = 0)
{
  auto attn = torch::ones({B * M * N}, options_fp16);
  auto attn_weights = torch::ones({N * N}, options_fp16);
  auto x = torch::nn::init::uniform_(
      torch::randn({B * M, N}, options_fp16), 0, 1);
  // auto x_mean = torch::
  auto x_mean = torch::mean(x, {
                                   1,
                               },
                            true);
  ...
  at::Half *ptr_attn = attn.data<at::Half>();
  at::Half *ptr_attn_weights = attn_weights.data<at::Half>();
  ...
  void *fused_feedforward_attn_fc1_kernel_args[] = {
      (void *)&(ptr_attn),
      (void *)&(ptr_attn_weights),
      (void *)&(ptr_x),
      (void *)&(ptr_x_mean),
  ...
  void *fused_feedforward_fc1_kernel_args[] = {
      (void *)&(ptr_x),
      (void *)&(ptr_x_mean),
      (void *)&(ptr_x_variance_sum),
      (void *)&(ptr_fc1_weight),
      (void *)&(ptr_fc1_output)};
  void *fused_feedforward_fc2_kernel_args[] = {
      (void *)&(ptr_fc1_output),
      (void *)&(ptr_fc2_weight),
      (void *)&(ptr_fc2_output),
      (void *)&(ptr_x)};
  ...
  const size_t fused_fc1_fc2_shared_memory = 32256;
  const size_t fuse_attn_fc1_shared_memory = 
  ...
  auto device_func = [&](int func_id)
  {
    ...
    case 0:
      AT_DISPATCH_FLOATING_TYPES_AND_HALF(fc2_output.type(), "feef_forward_fc1", [&]
        ...
        // dim3(128, 4, 1), dim3(32, 2, 4), fused_feedforward_fc1_kernel_args, fused_feed_forward_shared_memory));
    ...
    case 1:
      AT_DISPATCH_FLOATING_TYPES_AND_HALF(fc2_output.type(), "bert_attn_qkv", [&]
        ...
        // dim3(64, 2, 1), dim3(32, 2, 2), fused_feedforward_fc2_kernel_args, fused_feed_forward_shared_memory));
    ...
    case 2:
      AT_DISPATCH_FLOATING_TYPES_AND_HALF(fc2_output.type(), "bert_attn_qkv", [&]
                                          { checkCuda(cudaLaunchCooperativeKernel((void *)fused_feed_forward_fc1_fc2_m4096_n512_k128,
                                                                                  dim3(128, 4, 1), dim3(32, 2, 4), fused_feed_forward_fc1_fc2_kernel_args, fused_fc1_fc2_shared_memory)); });
      break;
    case 3:
      AT_DISPATCH_FLOATING_TYPES_AND_HALF(fc2_output.type(), "bert_attn_qkv", [&]
                                          { checkCuda(cudaLaunchCooperativeKernel((void *)fused_attn_fc1,
                                                                                  dim3(128, 4, 1), dim3(32, 2, 4), fused_feedforward_attn_fc1_kernel_args, fused_fc1_fc2_shared_memory)); });
    default:
      break;
    }
  };
