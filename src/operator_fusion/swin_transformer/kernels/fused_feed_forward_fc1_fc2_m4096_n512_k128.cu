...
// shared_memory: 23040
extern "C" __global__ void __launch_bounds__(256, 5) fused_feed_forward_fc1_fc2_m4096_n512_k128(half *__restrict__ x, half *__restrict__ x_mean, half *__restrict__ x_variance_sum, half *__restrict__ weight, half *__restrict__ compute,
                                                                                   half *__restrict__ weight_second, half *__restrict__ add, half *__restrict__ short_cut)
{
  ...
  half *weight_shared_second = ((half *)weight_shared) + 9216;
  half *compute_shared_second = ((half *)all_shared_mem);
  ...
  for (int k_outer_outer = 0; k_outer_outer < 2; ++k_outer_outer)
  {
    ...
    if (k_outer_outer == 1)
    {
      ...
      if (blockIdx.x < 64 && blockIdx.y < 2 && threadIdx.z < 2){
      for (int ax0_ax1_fused_outer_outer_outer_outer1 = 0; ax0_ax1_fused_outer_outer_outer_outer1 < 4; ++ax0_ax1_fused_outer_outer_outer_outer1)
      {
        ...
        cuda::memcpy_async(((uint4 *)(weight_shared_second + ((((((ax0_ax1_fused_outer_outer_outer_outer1 * 1152) + (((int)threadIdx.z) * 576)) + (((int)threadIdx.y) * 288)) + ((((int)threadIdx.x) >> 3) * 72)) + ((((int)threadIdx.x) & 7) * 8))))),
                           ((uint4 *)(weight_second + ((((((((((int)blockIdx.y) * 32768) + (ax0_ax1_fused_outer_outer_outer_outer1 * 8192)) + (((int)threadIdx.z) * 4096)) + (((int)threadIdx.y) * 2048)) + ((((int)threadIdx.x) >> 3) * 512)) + ((k_outer_outer - 1) * 64)) + ((((int)threadIdx.x) & 7) * 8))))),
                           shape, pipe);
      }
      }
  ...
  if (blockIdx.x < 64 && blockIdx.y < 2 && threadIdx.z < 2)
  {
    ...
    nvcuda::wmma::fragment<nvcuda::wmma::matrix_a, 8, 32, 16, half, nvcuda::wmma::row_major> compute_shared_second_wmma_matrix_a[4];
    nvcuda::wmma::fragment<nvcuda::wmma::matrix_b, 8, 32, 16, half, nvcuda::wmma::col_major> weight_shared_second_wmma_matrix_b[1];
    ...
    for (int k_outer_outer = 0; k_outer_outer < 8; ++k_outer_outer)
    {
      ...
      {
        ((uint4 *)(compute_shared_second + ((((((ax0_ax1_fused_outer_outer_outer_outer * 1152) + (((int)threadIdx.z) * 576)) + (((int)threadIdx.y) * 288)) + ((((int)threadIdx.x) >> 3) * 72)) + ((((int)threadIdx.x) & 7) * 8)))))[0] = ((uint4 *)(compute + ((((((((((int)blockIdx.x) * 32768) + (ax0_ax1_fused_outer_outer_outer_outer * 8192)) + (((int)threadIdx.z) * 4096)) + (((int)threadIdx.y) * 2048)) + ((((int)threadIdx.x) >> 3) * 512)) + (k_outer_outer * 64)) + ((((int)threadIdx.x) & 7) * 8)))))[0];
      }
      if (k_outer_outer > 0)
      {
        for (int ax0_ax1_fused_outer_outer_outer_outer1 = 0; ax0_ax1_fused_outer_outer_outer_outer1 < 4; ++ax0_ax1_fused_outer_outer_outer_outer1)
        {
          ((uint4 *)(weight_shared_second + ((((((ax0_ax1_fused_outer_outer_outer_outer1 * 1152) + (((int)threadIdx.z) * 576)) + (((int)threadIdx.y) * 288)) + ((((int)threadIdx.x) >> 3) * 72)) + ((((int)threadIdx.x) & 7) * 8)))))[0] = ((uint4 *)(weight_second + ((((((((((int)blockIdx.y) * 32768) + (ax0_ax1_fused_outer_outer_outer_outer1 * 8192)) + (((int)threadIdx.z) * 4096)) + (((int)threadIdx.y) * 2048)) + ((((int)threadIdx.x) >> 3) * 512)) + (k_outer_outer * 64)) + ((((int)threadIdx.x) & 7) * 8)))))[0];
        }
      }
      __syncthreads();
      for (int k_outer_inner = 0; k_outer_inner < 4; ++k_outer_inner)
      {
        ...
        {
          (void)nvcuda::wmma::load_matrix_sync(compute_shared_second_wmma_matrix_a[ax0_outer], ((half *)compute_shared_second + ((((((int)threadIdx.y) * 2304) + (ax0_outer * 576)) + (k_outer_inner * 16)))), 72);
        }
        (void)nvcuda::wmma::load_matrix_sync(weight_shared_second_wmma_matrix_b[0], ((half *)weight_shared_second + (((((int)threadIdx.z) * 2304) + (k_outer_inner * 16)))), 72);
        for (int i_c_outer = 0; i_c_outer < 4; ++i_c_outer)
        {
          (void)nvcuda::wmma::mma_sync(T_dense_wmma_accumulator[i_c_outer], compute_shared_second_wmma_matrix_a[i_c_outer], weight_shared_second_wmma_matrix_b[0], T_dense_wmma_accumulator[i_c_outer]);
        }
      }
    }
    __syncthreads();
    ...
    {
      (void)nvcuda::wmma::store_matrix_sync(((half *)compute_shared_second + ((((((int)threadIdx.y) * 2304) + (ax0_outer_inner * 576)) + (((int)threadIdx.z) * 32)))), T_dense_wmma_accumulator[ax0_outer_inner], 72, nvcuda::wmma::mem_row_major);
    }
    __syncthreads();
    ...
    {
      uint4 _1;
      uint4 _2 = ((uint4 *)(compute_shared_second + ((((((i_inner_j_inner_fused_outer_outer_outer_outer * 1152) + (((int)threadIdx.z) * 576)) + (((int)threadIdx.y) * 288)) + ((((int)threadIdx.x) >> 3) * 72)) + ((((int)threadIdx.x) & 7) * 8)))))[0];
