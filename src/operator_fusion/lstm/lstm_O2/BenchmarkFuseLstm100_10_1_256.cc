...
#include "lstm_cell_net_100_10/LSTM100_10.h"
...
int main(int argc, char *argv[]) {
    ...
    if(argc < 2){
        std::cout << "Usage: ./BenchmarkFuseLstm100_10_1_256 <path_to_test_data>" << std::endl;
        return 0;
    }
    std::string path_to_test_data = (std::string)argv[1];

    int fd = open((path_to_test_data + "/inputParams.bin").c_str(), O_CREAT | O_RDWR,
                  S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
    int result_fd = open((path_to_test_data + "/expectResult.bin").c_str(), O_CREAT | O_RDWR,
                         S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
