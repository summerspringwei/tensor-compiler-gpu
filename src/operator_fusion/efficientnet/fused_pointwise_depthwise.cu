...
extern "C" __global__ void __launch_bounds__(256) fused_pointwise_depthwise(
  ...
  // __shared__ float shared_depthwise_weight[tile_size_out_channels*kernel_height*kernel_width];//HWC
  ...
  // const int depth_tile_size_z = 256 / (tile_size_height*tile_size_width);
  // const int depth_num_iters = UPDIV(tile_size_out_channels, depth_tile_size_z);
  // float depthwise_local[depth_num_iters];
  // #pragma unroll
  // for(int i=0; i<depth_num_iters; ++i){
  //   depthwise_local[i] = 0;
  // }

  // const int depth_tz = threadIdx.x % depth_tile_size_z;
  // const int depth_tx = (threadIdx.x / depth_tile_size_z) / tile_size_width;
  // const int depth_ty = (threadIdx.x / depth_tile_size_z) % tile_size_width;
  
  // if(depth_tx >= tile_size_height){
  //   return;
  // }
  // // #pragma unroll
  //   for(int i=0; i<depth_num_iters; ++i) {
  ...
  //       int idx_intermedia = (depth_tx+kh)*padded_tile_size_width*tile_size_out_channels + (depth_ty+kw) * tile_size_out_channels + i * depth_tile_size_z + depth_tz;
  //       int idx_depth_weight = kh*kernel_width*tile_size_out_channels + kw*tile_size_out_channels + i*depth_tile_size_z + depth_tz;
  ...
  //     }
  //   }
  //   int idx_output = (bx * tile_size_height + depth_tx) * width * out_channels + (by * tile_size_width + depth_ty) * out_channels + bz * tile_size_out_channels + i * depth_tile_size_z + depth_tz;
  //   if(depthwise_local[i] == 0){
  //     output[idx_output] = depthwise_local[i];  
  //   }
  //   output[idx_output] = depthwise_local[i];
  // }
  ...
  for(int i=0; i<2; ++i){
    ...
    for(int j=0; j<4; ++j){
      ...
      if(output[(bx*tile_size_height+depth_tx*2+i)*width*out_channels + (by*tile_size_width+j)*out_channels + bz * tile_size_out_channels + depth_tz] <1){
        output[(bx*tile_size_height+depth_tx*2+i)*width*out_channels + (by*tile_size_width+j)*out_channels + bz * tile_size_out_channels + depth_tz] = 6.66;
      }
    }
  }
}
