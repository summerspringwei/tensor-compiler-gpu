...
__global__ void __launch_bounds__(kBlockSize) efficientnet_se_module_v2(
  float* input, float* reduce_sum_output, float* se_reduce_weight, float* se_reduce_output, 
  float* se_expand_weight, float* se_expand_output, long long* profile_grid_clock){
    cooperative_groups::grid_group grid = cooperative_groups::this_grid();
    int clock_idx = 0;
    const int kNumWarp = kBlockSize / warpSize;
    const int warpIdx = threadIdx.x / warpSize;

    profile_grid_clock[clock_idx * kBlockSize * kNumWarp + blockIdx.x * kNumWarp + warpIdx] = clock64(); clock_idx++;
    ...
    if (blockIdx.x < in_channel/tile_size_in_channel){
      ...
      for(int j=0; j<tile_size_in_channel; ++j){
        ...
        for(int i=0; i<kImgIter - 1; ++i) {
          ...
          const int global_idx = blockIdx.x * tile_size_in_channel * kImageSize + j * kImageSize + i * kBlockSize + threadIdx.x;
          shared_input[shared_idx] = input[global_idx];
        }
        // Load left image pixels
        if(threadIdx.x < (kImageSize % kBlockSize)){
          const int shared_idx = j * kPadImgSize + (kImgIter-1)*kBlockSize + threadIdx.x;
          const int global_idx = blockIdx.x * tile_size_in_channel * kImageSize + j * kImageSize + (kImgIter-1) * kBlockSize + threadIdx.x;
          shared_input[shared_idx] = input[global_idx];
        }
        ...
        if(threadIdx.x == 0){
          reduce_sum_output[blockIdx.x * tile_size_in_channel + j] = result / kImageSize;
        }
        __syncthreads();
      }
    }
    ...
    if(blockIdx.x < reduce_channel){
      ...
      for(int i=0; i<kIterInChannel; ++i){
        ...
        shared_reduce_weight[reduce_idx] = se_reduce_weight[blockIdx.x * in_channel + reduce_idx];
      }
  ...
  //         auto matmul2_weight = se_expand_weight[(blockIdx.x * tile_size_in_channel + warpId) * reduce_channel + i*warpSize+threadIdx.x];
  //         mul_result[i] = matmul1_output * matmul2_weight;
  //       }
...
template<int64_t batch, int64_t height, int64_t width, 
  int64_t in_channel, int64_t reduce_channel, int64_t tile_size_in_channel>
__global__ void __launch_bounds__(kBlockSize) efficientnet_se_module_v2_avg_pool(
  float* input, float* reduce_sum_output, float* se_reduce_weight, float* se_reduce_output, 
  float* se_expand_weight, float* se_expand_output, long long* profile_grid_clock){
    const int kNumWarp = kBlockSize / warpSize;
    const int warpIdx = threadIdx.x / warpSize;
...
template<int64_t batch, int64_t height, int64_t width, 
  int64_t in_channel, int64_t reduce_channel, int64_t tile_size_in_channel>
__global__ void __launch_bounds__(kBlockSize) efficientnet_se_module_v2_avg_pool_v2(
  float* input, float* reduce_sum_output, float* se_reduce_weight, float* se_reduce_output, 
  float* se_expand_weight, float* se_expand_output, long long* profile_grid_clock){
    const int kNumWarp = kBlockSize / warpSize;
    const int warpIdx = threadIdx.x / warpSize;
    ...
    for(int i=0; i<tile_size_in_channel; ++i){
      for(int j=0; j<kPadImgSize/kBlockSize; ++j){
        ...
        const int global_idx = blockIdx.x * tile_size_in_channel * kImageSize + img_idx;
    ...
    for(int i=0; i<tile_size_in_channel; ++i){
      ...
      if(threadIdx.x == 0){
        reduce_sum_output[blockIdx.x * tile_size_in_channel + i] = sum / kImageSize;
      }
    }
}
...
template<int64_t batch, int64_t height, int64_t width, 
  int64_t in_channel, int64_t reduce_channel, int64_t tile_size_in_channel>
__global__ void __launch_bounds__(kBlockSize) efficientnet_se_module_v2_matmul1(
  float* input, float* reduce_sum_output, float* se_reduce_weight, float* se_reduce_output, 
  float* se_expand_weight, float* se_expand_output, long long* profile_grid_clock){
    const int kNumWarp = kBlockSize / warpSize;
    const int warpIdx = threadIdx.x / warpSize;
...
template<int64_t batch, int64_t height, int64_t width, 
  int64_t in_channel, int64_t reduce_channel, int64_t tile_size_in_channel>
__global__ void __launch_bounds__(kBlockSize) efficientnet_se_module_v2_matmul2(
  float* input, float* reduce_sum_output, float* se_reduce_weight, float* se_reduce_output, 
  float* se_expand_weight, float* se_expand_output, long long* profile_grid_clock){
  // (3) Third Op matmul: (N, RC) * (RC, IC) -> (N, RC)
  // Each block process RC*tile_size_in_channel
      ...
      if(warp_iter_idx < tile_size_in_channel){
        ...
        for(int i=0; i<UPDIV(reduce_channel, warpSize); ++i){
          ...
          if(idx < reduce_channel){
            ...
            auto matmul2_weight = se_expand_weight[(blockIdx.x * tile_size_in_channel + warp_iter_idx) * reduce_channel + idx];
            sum += (matmul1_output * matmul2_weight);
          }
        }
        sum = warpReduceSum(sum);
        if((threadIdx.x % 32) == 0){
          se_expand_output[(blockIdx.x * tile_size_in_channel + warp_iter_idx)] = sum;
        }
      }
    }
  }
