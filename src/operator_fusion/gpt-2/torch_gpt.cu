...
float test_bert_attn(int round_cout = 1, int loop = 1, int func_id = 0) {
  ...
  // 1. Load weight from model file
  std::string folder_path = "/home/xiachunwei/Projects/tensor-compiler-gpu/src/operator_fusion/gpt-2/";
  torch::Tensor qkv_weight = torch_load_tensor(folder_path + "gpt2-torch-data/c_attn.pt")
                                 .to(torch::kCUDA)
                                 .to(torch::kHalf);
  ...
  torch::Tensor src =
      torch_load_tensor(folder_path + "gpt2-torch-data/attn_input_hidden_states.pt")
          .to(torch::kCUDA)
          .to(torch::kHalf);
  torch::Tensor feedforward_input =
  ...
  //   torch::ones({batch_size*num_heads, max_seq_length, hidden_size},
  //   options_fp16), torch::tensor({scale,}, options_fp16));
...
//   auto bias_qkv = torch::zeros({3, d_model}, options_fp16);
  ...
  // fused QKV matmul
//   auto batched_src =
//       torch::reshape(src.repeat({3, 1, 1}), {3, max_seq_length, d_model});
//   auto t_output_qkv = torch::permute(
//       torch::reshape(torch::bmm(batched_src, qkv_weight),
//                      {3, max_seq_length, num_heads, hidden_size}),
//       {0, 2, 1, 3}); //(3, num_heads, max_seq_length, hidden_size)
//   auto qkv = torch::split(t_output_qkv, 1, 0);
  ...
  // auto t_query = torch::ones({num_heads, max_seq_length, hidden_size},
  // options_fp16); auto t_key = torch::nn::init::uniform_(
  //   torch::randn({num_heads, max_seq_length, hidden_size}, options_fp16));
  // auto t_value = torch::nn::init::uniform_(
  //   torch::randn({num_heads, max_seq_length, hidden_size}, options_fp16));
  // auto t_query = torch::reshape(qkv[0], {num_heads, max_seq_length,
  // hidden_size}); auto t_key = torch::reshape(qkv[1], {num_heads,
  // max_seq_length, hidden_size});
//   auto t_value =
//       torch::reshape(qkv[2], {num_heads, max_seq_length, hidden_size});
//   auto cloned_query = torch::reshape(
//       qkv[0], {batch_size * num_heads, max_seq_length, hidden_size});
//   auto cloned_key = torch::reshape(
//       qkv[1], {batch_size * num_heads, max_seq_length, hidden_size});
//   // auto t_query_key_output = t_query.bmm(torch::permute(t_key, {0, 2, 1}));
//   auto t_query_key_output =
//       cloned_query.bmm(torch::permute(cloned_key, {0, 2, 1}));
//   auto t_attn_mask = torch::zeros(
//       {batch_size * num_heads, max_seq_length, max_seq_length}, options_fp16);
//   float v_d_model[] = {
//       d_model,
//   };
//   auto t_d_model = torch::from_blob(v_d_model,
//                                     {
//                                         1,
//                                     })
//                        .to(torch::kCUDA);
//   auto t_query_key_output_sum = torch::sum(
//       torch::exp(t_query_key_output / torch::sqrt(t_d_model)) + t_attn_mask,
//       -1);
//   auto t_query_key_softmax = torch::softmax(
//       (t_query_key_output / torch::sqrt(t_d_model)) + t_attn_mask, -1,
//       torch::kHalf);
//   auto t_attn_value_output =
//       torch::bmm(t_query_key_softmax, t_value); // Now (12, 384, 64)
//   auto t_attn_value_output_permuted =
//       torch::reshape(torch::permute(t_attn_value_output, {1, 0, 2}),
//                      {max_seq_length, d_model});
//   auto t_attn_fc_output =
//       torch::matmul(t_attn_value_output_permuted, attn_fc_weight);
//   auto t_attn_fc_short_cut_add = t_attn_fc_output + src;
//   auto t_attn_fc_layer_norm_x_2 =
//       torch::sum(t_attn_fc_short_cut_add * t_attn_fc_short_cut_add, 1);
//   auto t_attn_fc_layer_norm_x = torch::sum(t_attn_fc_short_cut_add, 1);
//   auto t_attn_fc_layer_norm_output =
//       torch::layer_norm(t_attn_fc_short_cut_add, {
//                                                      d_model,
//                                                  });
  auto t_feed_forward_fc1_output =
      torch::matmul(t_attn_fc_layer_norm_output, feed_forward_fc1_weight);
//   auto t_feed_forward_fc1_activation_output =
//       torch::relu(t_feed_forward_fc1_output);
//   auto t_feed_forward_fc2_output = torch::matmul(
//       t_feed_forward_fc1_activation_output, feed_forward_fc2_weight);
//   auto t_feed_forward_fc2_short_cut_output =
//       t_feed_forward_fc2_output + t_attn_fc_layer_norm_output;
//   auto t_feed_forward_fc2_layer_norm_sum_x_2 = torch::sum(
//       t_feed_forward_fc2_short_cut_output * t_feed_forward_fc2_short_cut_output,
//       1);
//   auto t_feed_forward_fc2_layer_norm_sum_x =
//       torch::sum(t_feed_forward_fc2_output, 1);
//   // auto t_feed_forward_fc2_short_cut_output = t_feed_forward_fc2_output +
//   // torch::ones({max_seq_length, d_model}, options_fp16);
//   auto t_feed_forward_fc2_layer_norm =
//       torch::layer_norm(t_feed_forward_fc2_short_cut_output, {
//                                                                  d_model,
//                                                              });
  printf("Torch finshed\n");

  // 3. Our implementation
  // 3.1 Allocate intermedia data
...
//       feed_forward_profile_clock.data_ptr<int64_t>();
  ...
  // Pointers from torch
//   at::Half *ptr_t_attn_fc_layer_norm_output =
//       t_attn_fc_layer_norm_output.data_ptr<at::Half>();
//   at::Half *ptr_t_feed_forward_fc2_output =
//       t_feed_forward_fc2_output.data_ptr<at::Half>();
//   at::Half *ptr_t_attn_fc_output = t_attn_fc_output.data_ptr<at::Half>();
//   at::Half *ptr_t_attn_fc_short_cut_add =
//       t_attn_fc_short_cut_add.data_ptr<at::Half>();
//   at::Half *ptr_t_feed_forward_fc2_short_cut_output =
//       t_feed_forward_fc2_short_cut_output.data_ptr<at::Half>();

//   half eps = 0.00001, gama = 1, beta = 0;
  // 4. My Fused bert
//   auto tmp_qkv_output = torch::ones({3, max_seq_length, d_model}, options_fp16);
//   auto tmp_query_key_output =
//       torch::ones({num_heads, max_seq_length, max_seq_length}, options_fp16);
//   at::Half *ptr_tmp_qkv_output = tmp_qkv_output.data_ptr<at::Half>();
//   at::Half *ptr_tmp_query_key_output =
//       tmp_query_key_output.data_ptr<at::Half>();
...
//   };
  ...
  {
      // 4.1 fused qkv matmul
      //   void *fused_attn_kernel_args[] = {(void *)&(ptr_weight_qkv),
      //                                     (void *)&(ptr_src), (void
      //                                     *)&(ptr_bias_qkv),
      //                                     (void *)&(ptr_output_qkv)};
      //   const size_t gemm_k1_shared_mem =
      //       (kStage(3 * kChunkK * kWmmaK *
      //                   (kBlockRowWarps * kGemmK1WarpRowTiles * kWmmaM +
      //                   kInputSkew) +
      //               kBlockColWarps * kGemmK1WarpColTiles * kWmmaN *
      //                   (kChunkK * kWmmaK + kInputSkew))) *
      //       sizeof(half);
      //   check_compatability(128, gemm_k1_shared_mem, (void
      //   *)gemm_add_qkv_bias);
      ...
      //         (void *)gemm_add_qkv_bias, dim3(24 * 4, 1, 1), dim3(128, 1, 1),
      //         fused_attn_kernel_args, gemm_k1_shared_mem));
      //   };
  }

  {
      // 4.2 query key matmul
      //   auto t_ptr_key = cloned_key.data_ptr<at::Half>();
      //   auto t_ptr_query = cloned_query.data_ptr<at::Half>();
      //   void *fused_attn_query_key_kernel_args[] = {(void *)&(t_ptr_key),
      //                                               (void *)&(t_ptr_query),
      //                                               (void
      //                                               *)&(ptr_query_key_output)};
      //   const int gemm_k2_blocks =
      //       (max_seq_length / (kBlockRowWarps * kGemmK2WarpRowTiles *
      //       kWmmaM)) * /*3*/
      //       (max_seq_length / (kBlockColWarps * kGemmK2WarpColTiles *
      //       kWmmaN)) * /*3*/
  ...
  {
      // 4.3 qk value
      //   auto ptr_value = t_value.data_ptr<at::Half>();
      //   auto ptr_qk = t_query_key_output.data_ptr<at::Half>();
      ...
      //       (kStage *
      //        (kChunkK * kWmmaK *
      //             (kBlockRowWarps * kGemmK3WarpRowTiles * kWmmaM +
      //             kInputSkew) +
      //         kBlockColWarps * kGemmK3WarpColTiles * kWmmaN *
      //             (kChunkK * kWmmaK + kInputSkew))) *
      //       sizeof(half);
  ...
  {
    // 4.4 attn_fc, inputA: (768, 768), inputB: (384, 768), C(384, 768)
    //   void *fused_attn_value_kernel_args[] = {(void *)&(ptr_value),
    //                                           (void *)&(ptr_qk),
    //                                           (void
    //                                           *)&(ptr_attn_value_output)};
    ...
    //                                        (void *)&(ptr_attn_fc_output)};
    //   checkCuda(cudaFuncSetAttribute(
    //       (const void *)gemm_three_stage<kGemmK4WarpRowTiles,
    //       kGemmK4WarpColTiles,
    //                                      d_model, max_seq_length, d_model,
    //                                      1>,
    //       cudaFuncAttribute::cudaFuncAttributeMaxDynamicSharedMemorySize,
    //       gemm_k4_shared_mem));
  }
  ...
  {
    // 4.5 feed_forward_fc1 inputA:(768,3072), inputB: (384,768),
    // output:(384,3072)
    ...
    fused_feed_forward_fc1 = [&]() {
      checkCuda(cudaLaunchCooperativeKernel(
          ...
          fused_feed_forward_fc1_kernel_args, feed_forward_fc1_shared_mem), __LINE__);
    };
    ...
    torch::print(feed_forward_fc1_output);
    printf("feed_forward_fc1_output\n");
  }

  {
      // 4.6 feed_forward_fc2 inputA:(3072,768), inputB: (384,3072),
      // output:(384,768)
  ...
  {
      // 4.7 Fused_feed_forward
      // we use the result from torch
      //   auto tmp_feed_forward_fc1_output =
      //       torch::ones({batch_size * max_seq_length, d_intermedia},
      //       options_fp16);
      //   at::Half *ptr_tmp_feed_forward_fc1_output =
      //       tmp_feed_forward_fc1_output.data_ptr<at::Half>();
      //   auto tmp_feed_forward_fc2_weight =
      //       torch::ones({d_intermedia, d_model}, options_fp16);
      //   at::Half *ptr_tmp_feed_forward_fc2_weight =
      //       tmp_feed_forward_fc2_weight.data_ptr<at::Half>();

      //   void *fused_feedforward_kernel_args[] = {
      //       // (void *)&(ptr_attn_fc_output),
      //       (void *)&(ptr_t_attn_fc_layer_norm_output), (void *)&(eps),
      //       (void *)&(gama), (void *)&(beta), (void
      //       *)&(ptr_feed_forward_fc1_weight),
      ...
      //       gemm_k5_shared_mem + kStage *
      //                                (kGemmK6BlockSliceKTiles * kWmmaK *
      //                                 (kGemmK6BlockRowTiles * kWmmaM +
      //                                 kInputSkew)) *
      //                                sizeof(half2);
      //   checkCuda(cudaFuncSetAttribute(
  ...
  {
    // 4.8 For debug single kernel performance
    //   checkCuda(cudaFuncSetAttribute(
    //       (const void *)debug_feed_forward_fc1,
    //       cudaFuncAttribute::cudaFuncAttributeMaxDynamicSharedMemorySize,
    //       120 * 1024));
    //   checkCuda(cudaFuncSetAttribute(
    //       (const void *)debug_fused_sqq_bert_pipelined,
    //       cudaFuncAttribute::cudaFuncAttributeMaxDynamicSharedMemorySize,
    //       fused_bert_shared_mem));
  }
  ...
  auto device_func = [&](int func_id) {
    ...
    case 0:
      ...
      //   [&] {
      //     checkCuda(cudaLaunchCooperativeKernel(
      //         (void *)fused_sqq_bert, dim3(108, 1, 1), dim3(128, 1, 1),
      //         fused_bert_kernel_args, fused_bert_shared_mem));
      //   });
      break;
    case 1:
      //   AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      //       output_qkv.type(), "func_attn_qkv", func_attn_qkv);
      break;
    case 2:
      //   AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      //       query_key_output.type(), "bert_attn_query_key", [&] {
      //         checkCuda(cudaLaunchCooperativeKernel(
    ...
    case 3:
      //   AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      //       output_qkv.type(), "bert_attn_value", [&] {
      //         checkCuda(cudaLaunchCooperativeKernel(
    ...
    case 4:
      //   AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      //       output_qkv.type(), "bert_attn_fc", [&] {
      //         gemm_three_stage<kGemmK4WarpRowTiles, kGemmK4WarpColTiles,
      //         d_model,
      //                          max_seq_length, d_model, 1>
      //             <<<dim3(gemm_k4_blocks, 1, 1), dim3(128, 1, 1)>>>(
      //                 (half *)ptr_attn_fc_weight, (half
      //                 *)ptr_attn_value_output, (half *)ptr_attn_fc_output);
      //       });
      break;
    case 5:
      AT_DISPATCH_FLOATING_TYPES_AND_HALF(
          feed_forward_fc1_output.type(), "bert_feed_forward_fc1", fused_feed_forward_fc1);
      break;
    case 6:
      //   AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      //       output_qkv.type(), "bert_feed_forward_fc2", [&] {
      //         checkCuda(cudaLaunchCooperativeKernel(
    ...
    case 7:
      //   AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      //       output_qkv.type(), "bert_fused_feed_forward", [&] {
      //         checkCuda(cudaLaunchCooperativeKernel(
      //             (const void *)fused_sqq_feedforward, dim3(gemm_k5_blocks,
      //             1, 1), dim3(128, 1, 1), fused_feedforward_kernel_args,
      //             fused_bert_shared_mem));
      //       });
      break;
    case 8:
      //   AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      //       output_qkv.type(), "bert_fused_feed_forward_pipelined", [&] {
      //         checkCuda(cudaLaunchCooperativeKernel(
      //             (const void *)fused_sqq_feedforward_pipelined,
    ...
    case 9:
      //   AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      //       output_qkv.type(), "fused_sqq_bert_pipelined_v2", [&] {
      //         checkCuda(cudaLaunchCooperativeKernel(
      //             (const void *)fused_sqq_bert_pipelined_v2, dim3(108, 1, 1),
      //             dim3(128, 1, 1), fused_bert_kernel_args,
      //             fused_bert_shared_mem));
      //       });
      break;
    case 10:
    ...
    case 11:
      //   AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      //       output_qkv.type(), "debug_fused_sqq_bert_pipelined", [&] {
      //         checkCuda(cudaLaunchCooperativeKernel(
      //             (const void *)debug_fused_sqq_bert_pipelined, dim3(108, 1,
      //             1), dim3(128, 1, 1), fused_bert_kernel_args,
      //             fused_bert_shared_mem));
      //       });
      break;
    case 12:
      //   AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      //       output_qkv.type(), "fused_sqq_feedforward_pipelined_v2", [&] {
      //         checkCuda(cudaLaunchCooperativeKernel(
    ...
    case 13:
      //   AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      //       output_qkv.type(), "attn_feed_forward", [&] {
      //         checkCuda(cudaLaunchCooperativeKernel(
      //             (const void *)fused_sqq_bert_attn, dim3(108, 1, 1),
      //             dim3(128, 1, 1), fused_bert_attn_kernel_args,
      //             fused_bert_shared_mem));
      //         checkCuda(cudaLaunchCooperativeKernel(
      //             (const void *)fused_sqq_feedforward_pipelined_v2,
      //             dim3(gemm_k5_blocks, 1, 1), dim3(128, 1, 1),
    ...
    case 14:
      ...
      //       output_qkv.type(), "attn_feed_forward", [&] {
      //         checkCuda(cudaLaunchCooperativeKernel(
      //             (const void *)fused_sqq_bert_query_key_softmax, dim3(108,
      //             1, 1), dim3(128, 1, 1), fused_bert_attn_kernel_args,
      //             fused_bert_shared_mem));
      //         checkCuda(cudaLaunchCooperativeKernel(
      //             (const void *)fused_sqq_feedforward_pipelined_v2,
      //             dim3(gemm_k5_blocks, 1, 1), dim3(128, 1, 1),
  ...
  // Check result
//   auto value = torch::reshape(torch::split(output_qkv, 1, 0)[2],
//                               {num_heads, max_seq_length, hidden_size});
  // my_compare(t_value, value, 1.0/16, 1.0/1024, compare_level);
  // my_compare(t_query_key_output_sum, query_key_softmax_sum, 1.0/16, 1.0/1024,
  // compare_level); my_compare(t_query_key_softmax,
  // query_key_output, 1.0/16, 1.0/1024, compare_level);
  // my_compare(t_attn_value_output_permuted,
  // attn_value_output, 1.0/16, 1.0/1024, compare_level);
  ...
  // my_compare(t_attn_fc_layer_norm_x,
  // attn_fc_layer_norm_x_2, 1.0/16, 1.0/1024, 2);
  // my_compare(t_attn_fc_layer_norm_x_2, layer_norm_variance, 1.0/16, 1.0/1024,
  // 2); my_compare(t_attn_fc_layer_norm_output,
  // attn_fc_output, 1.0/16, 1.0/1024, compare_level);
  // my_compare(t_feed_forward_fc1_activation_output,
  // feed_forward_fc1_output, 1.0/16, 1.0/1024, compare_level);
  // my_compare(t_feed_forward_fc2_output,
  // feed_forward_fc2_output, 1.0/16, 1.0/1024, 2);
  // my_compare(t_feed_forward_fc2_short_cut_output,
  // feed_forward_fc2_output, 1.0/16, 1.0/1024, 2);
  // my_compare(t_feed_forward_fc2_layer_norm_sum_x_2,
  // feed_forward_layer_norm_variance, 1.0/16, 1.0/1024, compare_level);
  // my_compare(t_feed_forward_fc2_layer_norm_sum_x,
  // feed_forward_layer_norm_sum, 1.0/16, 1.0/1024, compare_level);
  // my_compare(t_feed_forward_fc2_layer_norm,
  // feed_forward_fc2_output, 1.0/16, 1.0/1024, compare_level);
...
int main(int argc, char **argv) {
  ...
  test_bert_attn<1, 20, 384, 64, 5120>(round, loop, type);
  return 0;
}
