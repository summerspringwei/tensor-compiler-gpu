...
float test_bert_attn(int round_cout = 1, int loop = 1, int func_id = 0) {
  ...
  torch::Tensor qkv_weight = torch_load_tensor("gpt2-torch-data/c_attn.pt")
                                 .to(torch::kCUDA)
                                 .to(torch::kHalf);
  ...
  torch::Tensor feed_forward_fc1_weight =
      torch_load_tensor("gpt2-torch-data/MLP_c_fc.pt")
          .to(torch::kCUDA)
          .to(torch::kHalf);
  torch::Tensor feed_forward_fc2_weight =
      torch_load_tensor("gpt2-torch-data/MLP_c_proj.pt")
          .to(torch::kCUDA)
          .to(torch::kHalf);
  // Load input from file
  torch::Tensor src =
      torch_load_tensor("gpt2-torch-data/attn_input_hidden_states.pt")
          .to(torch::kCUDA)
          .to(torch::kHalf);
  torch::Tensor feedforward_input =
      torch_load_tensor("gpt2-torch-data/MLP_input_hidden_states.pt")
          .to(torch::kCUDA)
          .to(torch::kHalf);
  // Create dummy data
  // auto src = torch::div(
  ...
  at::Half *ptr_tmp_query_key_output = tmp_query_key_output.data_ptr<at::Half>();
...
//                                     (void *)&(ptr_src), (void *)&(ptr_bias_qkv),
//                                     (void *)&(ptr_output_qkv)};
...
//       gemm_k4_shared_mem));
  ...
  {
  // 4.6 feed_forward_fc2 inputA:(3072,768), inputB: (384,3072), output:(384,768)
...
//       torch::ones({batch_size * max_seq_length, d_intermedia}, options_fp16);
...
//       (void *)&(gama), (void *)&(beta), (void *)&(ptr_feed_forward_fc1_weight),
...
//       fused_bert_shared_mem));
  ...
  auto device_func = [&](int func_id) {
    ...
    //                 (half *)ptr_attn_fc_weight, (half *)ptr_attn_value_output,
    //                 (half *)ptr_attn_fc_output);
    //       });
      break;
    case 5:
      AT_DISPATCH_FLOATING_TYPES_AND_HALF(
    ...
    //         checkCuda(cudaLaunchCooperativeKernel(
    //             (const void *)fused_sqq_feedforward, dim3(gemm_k5_blocks, 1, 1),
    //             dim3(128, 1, 1), fused_feedforward_kernel_args,
    //             fused_bert_shared_mem));
    //       });
      break;
    case 8:
    ...
    //         checkCuda(cudaLaunchCooperativeKernel(
    //             (const void *)debug_fused_sqq_bert_pipelined, dim3(108, 1, 1),
    //             dim3(128, 1, 1), fused_bert_kernel_args,
    //             fused_bert_shared_mem));
    //       });
      break;
    ...
    //       output_qkv.type(), "attn_feed_forward", [&] {
    //         checkCuda(cudaLaunchCooperativeKernel(
    //             (const void *)fused_sqq_bert_query_key_softmax, dim3(108, 1, 1),
    //             dim3(128, 1, 1), fused_bert_attn_kernel_args,
    //             fused_bert_shared_mem));
    //         checkCuda(cudaLaunchCooperativeKernel(
    //             (const void *)fused_sqq_feedforward_pipelined_v2,
