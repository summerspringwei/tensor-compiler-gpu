...
void format_affine_transform_cpu(double* a, double* b, 
    ...
    // Replace get_3rd_point in python code
    ...
    #endif
    // Copy from opencv4.5 getAffineTransform
    for( int i = 0; i < 3; i++ ){
        int j = i*12;
        int k = i*12+6;
        a[j] = a[k+3] = src[i].x;
    ...
    // Get Affine transform by solving linear system
    // We implement using torch tensor api
}
